...
export function InstagramScrapingDialog({ isOpen, onClose }: InstagramScrapingDialogProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!selectedTerm) {
      toast.error("Por favor, selecciona un término de búsqueda.");
      return;
    }
    ...
    try {
      const response = await fetch(webhookUrl, {
        ...
        body: JSON.stringify({ search_term: selectedTerm }),
      });

      if (response.ok) {
        toast.success(`Solicitud de scraping para "${selectedTerm}" enviada correctamente.`);
        onClose();
      } else {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Selecciona un término para iniciar una búsqueda manual de leads en Instagram.
          </DialogDescription>
        </DialogHeader>
